...
user_settings = {
  user_spells = {
    ...
    Geomancy = T({ 'Indi-Refresh', 'Indi-Precision' }),
    Enhancing = T({ 'Protect', 'Shell' }),
    Ninjutsu = T({}),
    Singing = T({ "Army's Paeon IV", "Knight's Minne V", 'Sword Madrigal', 'Valor Minuet V' }),
    Blue = T({ 'Pollen' }),
    Summoning = T({ 'Carbuncle' }),
    Elemental = T({ 'Burn', 'Choke', 'Drown', 'Frost', 'Shock', 'Rasp' }),
  },
  save_settings = false,
...
} --put your string instrument here
sets.Idle = {
...
function get_sets()
  skilluprun = false
  gs_skill = {
    skillup_table = { 'Healing', 'Geomancy', 'Enhancing', 'Ninjutsu', 'Singing', 'Blue', 'Summoning', 'Elemental' },
    skillup_type = 'None',
    skillup_spells = T({}),
    skillup_count = 1,
    bluspellulid = { ['Harden Shell'] = 737, ['Pyric Bulwark'] = 741, ['Carcharian Verve'] = 745 },
    skill_up_item = T({ 5889, 5890, 5891, 5892 }),
  }
  ...
  gs_skillup.boxa = {
    pos = { x = gs_skillup.box.pos.x - 145, y = gs_skillup.box.pos.y },
...
function precast(spell)
  if pet.isvalid and S({ 'Release', 'Full Circle' }):contains(spell.en) then
    if spell.en == 'Release' then
      ...
      if recast > 0 then
        cancel_spell()
        send_command(
          'wait ' .. tostring(recast + 0.5) .. ';input /ja "' .. res.job_abilities[90][gearswap.language] .. '" <me>'
        )
      end
      return
    else
  ...
  elseif gs_skillup.use_geo then
    ...
    elseif
      player.sub_job == 'GEO'
      and spell_usable(res.spells[770])
      and buffactive[541] ~= (gs_skillup.use_trust and 2 or 1)
    then
...
function aftercast(spell)
  ...
  elseif spell.type == 'SummonerPact' then
    local spell_element = (
      type(spell.element) == 'number' and res.elements[spell.element][gearswap.language] or spell.element
    )
    if
      spell.en:contains('Spirit') and (spell_element == world.weather_element or spell_element == world.day_element)
    then
...
function check_skill_cap()
  if S({ 'Healing', 'Enhancing', 'Blue', 'Summoning', 'Elemental' }):contains(gs_skill.skillup_type) then
  ...
  elseif gs_skill.skillup_type == 'Singing' then
    if
      gs_skillup.skill['Singing Capped']
      and gs_skillup.skill['Stringed Instrument Capped']
      and gs_skillup.skill['Wind Instrument Capped']
      and not gs_skillup.test_mode
    then
      skilluprun = false
      return true
    end
...
function spell_valid(tab)
  if
    (
      tab.levels[player.main_job_id] and tab.levels[player.main_job_id] <= player.main_job_level
      or tab.levels[player.sub_job_id] and tab.levels[player.sub_job_id] <= player.main_job_level
    )
    and tab.targets:contains('Self')
    and not tab.en:wmatch('Teleport-*|Warp*|Tractor*|Retrace|Escape|Geo-*|Sacrifice|Odin|Alexander|Recall-*')
  then
    return true
  end
end
...
function updatedisplay()
  ...
  info.skillstring = (
    gs_skillup.skill['Stringed Instrument Capped'] and 'Capped' or gs_skillup.skill['Stringed Instrument Level']
  )
  info.skillwind = (
    gs_skillup.skill['Wind Instrument Capped'] and 'Capped' or gs_skillup.skill['Wind Instrument Level']
  )
  ...
  info.skill = {}
  info.skill.Healing = (
    gs_skillup.skill['Healing Magic Capped'] and 'Capped' or gs_skillup.skill['Healing Magic Level']
  )
  info.skill.Enhancing = (
    gs_skillup.skill['Enhancing Magic Capped'] and 'Capped' or gs_skillup.skill['Enhancing Magic Level']
  )
  info.skill.Elemental = (
    gs_skillup.skill['Elemental Magic Capped'] and 'Capped' or gs_skillup.skill['Elemental Magic Level']
  )
  info.skill.Summoning = gs_skillup.skill['Summoning Magic Capped'] and 'Capped'
    or gs_skillup.skill['Summoning Magic Level']
  ...
  info.ELEc = (gs_skillup.color.ELE and 'Start Elemental' or '\\cs(255,0,0)Start Elemental\\cr')
...
function file_write()
  ...
  file:write(
    'gs_skillup.box.pos.x = '
      .. tostring(gs_skillup.box.pos.x)
      .. '\ngs_skillup.box.pos.y = '
      .. tostring(gs_skillup.box.pos.y)
      .. '\ngs_skillup.boxa.pos.x = '
      .. tostring(gs_skillup.boxa.pos.x)
      .. '\ngs_skillup.boxa.pos.y = '
      .. tostring(gs_skillup.boxa.pos.y)
      .. ''
  )
  file:close()
end
