...
function get_sets()
    ...
    gs_skillup = {
        color = { GEO = true, HEL = true, ENH = true, ELE = true, NIN = true, SIN = true, BLU = true, SMN = true, STOP = true, DOWN = true, LOG = true, TRUST = true, TEST = true, REF = true, ITEM = true },
        skill_ups = {},
        total_skill_ups = 0,
        skill = {},
        use_trust = false,
    ...
    if pet.isvalid and S { "Release", "Full Circle" }:contains(spell.en) then
        if spell.en == "Release" then
            ...
            if (recast > 0) then
                cancel_spell()
                send_command('wait ' ..
                    tostring(recast + 0.5) .. ';input /ja "' .. res.job_abilities[90][gearswap.language] .. '" <me>')
            end
            return
        else
            return
...
function updatedisplay()
    ...
    info.skill.Summoning = gs_skillup.skill['Summoning Magic Capped'] and "Capped" or
        gs_skillup.skill['Summoning Magic Level']
...
windower.raw_register_event('action message',
